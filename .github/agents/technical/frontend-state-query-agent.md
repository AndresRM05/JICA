...
- Socket.io o notificaciones en tiempo real
- Redis o BullMQ
- Sentry
- Servicios cloud o de pago
El stack de estado permitido es:
```
...
Reportar si:
- Datos del backend (inversiones, simulaciones) están almacenados en Zustand en lugar de TanStack Query.
...
Los stores definidos para el MVP son tres:
 
| Store | Responsabilidad |
|---|---|
| `authStore` | Sesión, usuario autenticado y JWT token |
| `uiStore` | Tema, sidebar abierto/cerrado |
| `filterStore` | Filtros activos en el dashboard |
 
Reportar si:
- Se crea un store nuevo distinto a los tres definidos sin justificación documentada.
...
- El JWT token no está en `authStore` sino en `localStorage` o `sessionStorage`.
> No reportar como violación la ausencia de `notificationStore` — Socket.io está fuera del MVP.
...
- Se cachean datos del usuario autenticado con TanStack Query en lugar de `authStore`.
...
- Un proceso largo (simulación, confirmación de inversión) no muestra feedback visual con `isPending`.
